...
def cmd_list(a):
    ...
    if a.grep:
        g = a.grep.lower()
        ents = [e for e in ents if g in e.symbol.lower() or g in e.name]
    print(f"{'id':>5} {'offset':>10} {'size':>8} {'disc':>8} {'c':1} {'kind':10} {'tex':>4} symbol")
    for e in ents:
        print(f"{e.id:5d} {e.offset:#10x} {fmt_size(e.size):>8} {fmt_size(e.disc_size):>8} "
              f"{'z' if e.compressed else '-'} {e.kind:10} {e.ntex:4d} {e.symbol}")
